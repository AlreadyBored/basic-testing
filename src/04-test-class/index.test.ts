...
describe('BankAccount', () => {
  ...
  test('should throw InsufficientFundsError error when withdrawing more than balance', () => {
    ...
    expect(() => account.withdraw(200)).toThrowError('Insufficient funds: cannot withdraw more than 100');
  });
  ...
  test('should throw error when transferring more than balance', () => {
    ...
    expect(() => account1.transfer(200, account2)).toThrowError('Insufficient funds: cannot withdraw more than 100');
  });

  test('should throw error when transferring to the same account', () => {
    const account = getBankAccount(100);
    expect(() => account.transfer(100, account)).toThrowError(TransferFailedError);
    expect(() => account.transfer(100, account)).toThrowError('Transfer failed');
  });
