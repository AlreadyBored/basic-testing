...
describe('simpleCalculator tests', () => {
  ...
  test('should exponentiate two numbers', () => {
    ...
    expect(simpleCalculator({ a: 3, b: 4, action: Action.Exponentiate })).toBe(81);
    expect(simpleCalculator({ a: 5, b: 3, action: Action.Exponentiate })).toBe(125);
  });

  test('should return null for invalid action', () => {
    expect(simpleCalculator({ a: 1, b: 2, action: 'Invalid' as Action })).toBe(null);
    expect(simpleCalculator({ a: 2, b: 3, action: 'Invalid' as Action })).toBe(null);
    expect(simpleCalculator({ a: 3, b: 4, action: 'Invalid' as Action })).toBe(null);
    expect(simpleCalculator({ a: 5, b: 3, action: 'Invalid' as Action })).toBe(null);
  });

  test('should return null for invalid arguments', () => {
    expect(simpleCalculator({a: 'three', b: 2, action: Action.Add})).toBe(null);
  });
});
